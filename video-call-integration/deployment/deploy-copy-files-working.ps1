# Video Call Integration - File Copy Script
# Copy all necessary files to VM for deployment
...
ssh "$VMUser@$VMHost" "mkdir -p $VMBasePath/video-call-integration/server"
ssh "$VMUser@$VMHost" "mkdir -p $VMBasePath/video-call-integration/database"
ssh "$VMUser@$VMHost" "mkdir -p $VMBasePath/video-call-integration/client-components"
ssh "$VMUser@$VMHost" "mkdir -p $VMBasePath/video-call-integration/deployment"
ssh "$VMUser@$VMHost" "mkdir -p $VMBasePath/video-call-integration/nginx"
ssh "$VMUser@$VMHost" "mkdir -p $VMBasePath/video-call-integration/logs"
...
ssh "$VMUser@$VMHost" "chmod +x $VMBasePath/video-call-integration/deployment/*.sh"
ssh "$VMUser@$VMHost" "chmod +x $VMBasePath/video-call-integration/deployment/*.ps1"
ssh "$VMUser@$VMHost" "find $VMBasePath/video-call-integration -type f -name '*.js' -exec chmod 644 {} \;"
ssh "$VMUser@$VMHost" "find $VMBasePath/video-call-integration -type f -name '*.json' -exec chmod 644 {} \;"
